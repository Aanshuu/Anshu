...
const HomePage: FC = () => {
  ...
  const items = [
    <div key="1" className="h-full w-full flex items-center justify-center bg-primary dark:bg-black">
      <RevealText/>
    </div>,
    <div key="2" className="h-full w-full flex flex-col items-center justify-center bg-primary dark:bg-black">
      {/* Language Stats */}
      <img
        src={theme === "dark" ? githubStats.languageLight : githubStats.languageDark}
        alt="Language Stats"
        className="w-full max-w-lg overflow-hidden"
        loading="lazy" // Lazy load for performance
      />
    </div>,
    <div key="3" className="h-full w-full flex flex-col items-center justify-center bg-primary dark:bg-black">
      {/* GitHub Stats: Switch between preloaded dark and light theme images */}
      <img
        src={theme === "dark" ? githubStats.streakLight : githubStats.streakDark}
        alt="GitHub Stats"
        className="w-full max-w-md"
        loading="lazy" // Lazy load for performance
      />
    </div>,
    <div key="4" className="h-full w-full flex items-center justify-center bg-primary dark:bg-black">
      <h1 className="text-3xl font-bold text-black dark:text-[#fecfc5]">More Content</h1>
    </div>,
  ];
